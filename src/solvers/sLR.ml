...
struct
  ...
  struct 
    ...
    let fresh_key x = not (HM.mem keys x)
  ...
  struct
    ...
    struct
      type t = S.Var.t
      let compare x y = Int.compare (X.get_key_opt x) (X.get_key_opt y)
    end
  ...
  let solve box st list = 
    ...
    and backeval x y =   
       if (!Errormsg.debugFlag) then ignore (Pretty.printf "backeval: %a\n" S.Var.pretty_trace y);
